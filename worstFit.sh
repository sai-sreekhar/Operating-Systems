    ...
    local size=$2
    local worstFit=-1
    local worstFitSize=-1
    for key in "${!memory[@]}"; do
    local blockSize=${memory[$key]}
    if [[ $blockSize -ge $size ]]; then
    if [[ $worstFit -eq -1 || $blockSize -gt $worstFitSize ]]; then
    worstFit=$key
    worstFitSize=$blockSize
    fi
    fi
    done
    if [[ $worstFit -eq -1 ]]; then
    echo "Insufficient memory to allocate $size KB for process $process"
    return
    fi
    local remainingSize=$((worstFitSize - size))
    if [[ $remainingSize -gt 0 ]]; then
    memory[$worstFit]=$remainingSize
    echo "Process $process allocated $size KB of memory starting at address $worstFit"
    echo "Remaining block size at address $worstFit: $remainingSize KB"
    else
    unset memory[$worstFit]
    echo "Process $process allocated $size KB of memory starting at address $worstFit"
    fi
    }
